...
betac
```
The parameter mu stands for the recovery rate. It is the rate at which infected individuals recover and become immune to the disease. The epidemic threshold is defined as the minimum value of the transmission rate β required for an epidemic to occur in a population. 

Assuming mu = 0.1, the epidemic threshold in our network is very small (0.0039), because of the heterogeneity in the degree distribution (⟨k⟩ = 7.3 and ⟨k^2⟩ = 185.4, so ⟨k^2⟩>>⟨k⟩)
Therefore, it is very easy to spread in this network!
...
```
We can see than infecting first the nodes with higher centrality the peak is reached sooner in time, and the number of nodes infected is algo higher than with random spread. Thus, selecting the seeds according to centrality metrics does acceleate the spreading. 
...
Random vaccination has an effect similar to no vaccination at all with regard to the time at which the peak is reached. However, the infected nodes in the peak is much higher with no vaccination than with random.

On the contrary, targeted vaccination based on Page Rank centrality measure does more than halve the peak of infected people (from 4000 to 1500), which is reached later in time (at time 5 instead of time 2.5)
...
According to the graphs obtained in part c) and d), when we select the seeds according to higher _page rank_, the spreading gets faster(c). In the same way, when we target immunization also by page rank, we prevent spreading and the peak is reached later in time(d). 
