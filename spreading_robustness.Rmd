...
To build a model that predicts the time to infection of a node based on its degree, centrality, betweenness, and page rank, we adopted a multiclass classification approach. Initially, we compiled a dataset containing the necessary characteristics for each node. Next, we partitioned the dataset into separate train and test subsets to assess model performance. For the classification task, we opted for the random forest algorithm. Lastly, we conducted an evaluation of the model to examine its predictive capabilities and assess its effectiveness.
...
confusionMatrix(predictions, testing$t)
