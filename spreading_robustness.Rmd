...
The graph of variable importance for the random forest model predicting the time to infection of a node give us insights into the relative significance of different variables in determining the time of infection. Page rank emerges as the most influential variable, indicating that the ranking of nodes based on their importance in the network has a strong impact on the timing of infection. Closeness is the second most important variable, suggesting that nodes with higher closeness centrality, meaning they are more closely connected to other nodes in the network, tend to exhibit specific patterns in the timing of infection. Betweenness is the third important variable, indicating that nodes with higher betweenness centrality, which act as critical bridges connecting different parts of the network, have an influence on the timing of infection. Degree is the least important variable among the four, implies that the number of connections a node has in the network has a relatively smaller impact on the timing of infection. However, it still contributes to the model's prediction.

Selecting nodes of shortest time to infection:
```{r}
testing$predicted_ctg = predictions
seeds <- testing %>% filter(predicted_ctg == "Very Fast") %>% pull(id)
head(seeds)
```

In addition to the model previously explained, we created two alternative models that were discarded due to their low performance for influence maximization. The first alternative is a classification model that identifies high and low central nodes using its metrics. The second one is a linear regression that uses the time of spreading as a continuous target variable. The code is provided below but has not been executed.
...
nodes_df = nodes_data %>%  
  mutate(
    category = as.factor(case_when(
      degree >= quantile(degree, probs = 0.9) |
      closeness >= quantile(closeness, probs = 0.9) |
      pageRank >= quantile(pageRank, probs = 0.9) |
      betweenness >= quantile(betweenness, probs = 0.95) ~ "High",
    TRUE ~ "Low"))) 
...
The influence maximization model, utilizing centrality metrics, resulted in a targeted vaccination strategy that reduced the number of infected people by 500 compared to random vaccination. However, the model did not affect the overall time of the spreading process.
