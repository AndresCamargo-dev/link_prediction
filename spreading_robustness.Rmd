...
Now, we can compare the realizations with the random seeds and with the seeds selected by page rank.
...
Then, we run the simulation with the 5% chosing according to their centrality (those nodes with largest closeness)
...
Random vaccination has an effect similar to no vaccination at all. On the contrary, targeted vaccination based on Page Rank centrality measure does more than halve the peak of infected people (from 4000 to 1500), which is reached later in time (at time 5 instead of time 2.5)



## e) Analysis of Inmunization

*Comment on the relationship between the nodes you found in part c) and d) using the same type of centrality. Are they related or completely different?*
